...
(function(){
 ...
 function render(){
  ...
  const todayCount=open.filter(t=>t.due_date===today).length,lateCount=open.filter(t=>t.due_date&&t.due_date<today).length;
  el('tasks-content').innerHTML=`<div class="daily-task-intro" aria-label="Resumo das tarefas"><div class="daily-task-count urgent"><strong>${todayCount}</strong><span>para hoje</span></div><div class="daily-task-count urgent"><strong>${lateCount}</strong><span>atrasadas</span></div><div class="daily-task-count"><strong>${open.length}</strong><span>pendentes</span></div><div class="daily-task-count"><strong>${done.length}</strong><span>concluídas</span></div></div><section class="daily-task-section" aria-labelledby="open-task-title"><div class="daily-task-section-head"><h2 id="open-task-title">A fazer</h2><span>${open.length}</span></div><div class="daily-task-list">${open.length?open.map(t=>card(t,false,today)).join(''):'<div class="daily-task-empty">Tudo em dia. Crie uma tarefa para começar.</div>'}</div></section><section class="daily-task-section" aria-labelledby="done-task-title"><div class="daily-task-section-head"><h2 id="done-task-title">Concluídas</h2><span>${done.length}</span></div><div class="daily-task-list">${done.length?done.map(t=>card(t,true,today)).join(''):'<div class="daily-task-empty">As tarefas concluídas aparecerão aqui.</div>'}</div></section>`;
 }
 async function load(){
